...
//- (void)dealloc
//{
//    [self zhc_registerForNotifications];
//    
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.inputViewHeightConstraint.constant = self.inputMessageBarView.preferredDefaultHeight;
    NSLog(@"This is viewWillAppear inputViewHeightConstraint - %f", self.inputMessageBarView.preferredDefaultHeight);
    [self.view layoutIfNeeded];
    ZHCWeakSelf;
    if (self.automaticallyScrollsToMostRecentMessage) {
        if (![APPDELEGATE isTapOnImage]) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [weakSelf scrollToBottomAnimated:NO];
            });
        }else{
            [APPDELEGATE setIsTapOnImage:NO];
        }
//        dispatch_async(dispatch_get_main_queue(), ^{
//            [weakSelf scrollToBottomAnimated:NO];
//        });
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    NSLog(@"This is viewWillDisappear inputViewHeightConstraint - %f", self.inputViewHeightConstraint.constant);
}
...
- (void)scrollToIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated
{
    ...
    NSLog(@"print---1---%f",self.messageTableView.contentInset.bottom);
    CGFloat cellHeight = [self tableView:self.messageTableView heightForRowAtIndexPath:indexPath];
    CGFloat maxHeightForVisibleMessage = CGRectGetHeight(self.messageTableView.bounds) - self.messageTableView.contentInset.top - self.messageTableView.contentInset.bottom - CGRectGetHeight(self.inputMessageBarView.bounds);
...
-(UITableViewCell *)tableView:(ZHCMessagesTableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([messagecell.messageReadStatus isEqualToString:@"read"]) {
        if ([messagecell.senderId isEqualToString:[NSUSERDEFAULT valueForKey:@"userID"]]) {
            cell.rightSideReadStatus.image = [UIImage imageNamed:@"blueTick"];
        }else{
            cell.leftSideReadStatus.image = nil;
        }
    }else if ([messagecell.messageReadStatus isEqualToString:@"reached"]) {
        if ([messagecell.senderId isEqualToString:[NSUSERDEFAULT valueForKey:@"userID"]]) {
            cell.rightSideReadStatus.image = [UIImage imageNamed:@"tick2"];
        }else{
            cell.leftSideReadStatus.image = nil;
        }
    }else{
        if ([messagecell.senderId isEqualToString:[NSUSERDEFAULT valueForKey:@"userID"]]) {
            cell.rightSideReadStatus.image = [UIImage imageNamed:@"tick3"];
        }else{
            cell.leftSideReadStatus.image = nil;
        }
    }
...
- (void)zhc_registerForNotifications
{
...
-(void)zhc_keyboardWillShown:(NSNotification *)notification
{
    ...
    [UIView animateWithDuration:animationDuration
                     ...
                     completion:nil];
    NSLog(@"This Show inputViewHeightConstraint - %f", self.inputMessageBarView.preferredDefaultHeight);
...
-(void)zhc_keyboardWillHiden:(NSNotification *)notification
{
    ...
    NSLog(@"This hide inputViewHeightConstraint - %f", self.inputMessageBarView.preferredDefaultHeight);
}
