...
- (void)presentPopupViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    ...
    if (self.popupViewController == nil) {
        ...
        viewControllerToPresent.view.layer.shadowOpacity = 0.5f;
        ...
        if (flag) { // animate
            ...
            [UIView animateWithDuration:ANIMATION_TIME delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
                ...
                blurView.alpha = self.useBlurForPopup ? 1.0f : 0.25f;
...
- (void)presentPopupViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag WithAlpha:(float)alpha completion:(void (^)(void))completion {
    ...
    if (self.popupViewController == nil) {
        ...
        if (flag) { // animate
            ...
            [UIView animateWithDuration:ANIMATION_TIME delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
                viewControllerToPresent.view.frame = finalFrame;
                viewControllerToPresent.view.alpha = finalAlpha;
                blurView.alpha = self.useBlurForPopup ? 1.0f : alpha;
            } completion:^(BOOL finished) {
                [self.popupViewController didMoveToParentViewController:self];
                [self.popupViewController endAppearanceTransition];
                [completion invoke];
